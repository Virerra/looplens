  ...
  async function getAiSuggestions() {
    ...
    try {
      const res = await fetch('/api/suggest', {
        ...
        body: JSON.stringify({ apiKey: p.apiKey, prompt }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? `HTTP ${res.status}`)
      if (data.text) setAiTip(data.text)
    } catch (e) {
...
function buildTips(p) {
  ...
  if (p.accumulation === 'full' && p.iterations > 5) {
    ...
    const rFull = simulateLoop({ model: p.selectedModel, iterations: p.iterations, systemTokens: p.systemTokens, initialContext: p.initialContext, inputPerIter: p.inputPerIter, outputPerIter: p.outputPerIter, toolCallsPerIter: p.toolCallsPerIter, toolTokens: p.toolTok, accumulation: 'full', windowSize: 4, caching: { enabled: false, prefixTokens: 0, hitRate: 0 } })
    const sv = rFull.totalCost - slide.totalCost
    if (sv > 0.0001) tips.push({ type: 'warn', title: '💡 Switch to sliding window',
      body: `Full accumulation over ${p.iterations} iterations is expensive. A 4-turn sliding window saves ~${fmtCost(sv)} (${Math.round(sv / rFull.totalCost * 100)}% off) per run.` })
  }

  const tTok = p.toolTok * p.toolCallsPerIter * p.iterations
  if (tTok > 0) {
    const tf = (tTok / 1e6) * p.selectedModel.input / r.totalCost
    if (tf > 0.2) tips.push({ type: 'warn', title: `🔧 Tool calls are ${Math.round(tf * 100)}% of spend`,
  ...
  if (!p.cacheEnabled) {
    ...
    if (sv > 0.0001) tips.push({ type: 'info', title: '⚡ Enable prompt caching',
  ...
  if (last > first * 2) tips.push({ type: 'info', title: '📈 Context growth is compounding',
    body: `First iteration: ${fmtCost(first)}. Last: ${fmtCost(last)}. Add early-exit criteria or checkpoint saves to reduce spend on long loops.` })

  if (p.mode === 'fanout' && p.numAgents > 8) tips.push({ type: 'warn', title: `⚠ ${p.numAgents} parallel subagents`,
    body: `Fan-out cost scales linearly. Every optimization multiplies ${p.numAgents}×. Consider DeepSeek V4 Flash ($0.14/$0.28 per 1M) for subagents — 35× cheaper than GPT-5.5 input.` })

  tips.push({ type: 'ok', title: '✅ DeepSeek auto-caching',
    body: 'DeepSeek V4 Flash caches context automatically by default. Cache-hit input drops to $0.003/1M — up to 47× cheaper than GPT-5.5 for repeated agent prefixes.' })
